...
header("Location: ventiquattresimi.php#" . $_GET['partita']);
...
function calcola($s1, $s2, $c1, $c2, $coppa) {
    ...
    $sql1 = "SELECT * from squadra WHERE squadra.ID = '$s1' AND squadra.ID_Campionato = '$c1'";
        ...
        if(($ga1+$gr1+$r1)>($ga2+$gr2+$r2)){
            $sql = "UPDATE QEL SET Eliminato=1 WHERE ID_Squadra='$s2' AND ID_Campionato='$c2'";
            $sql1 = "UPDATE QEL SET Eliminato=0 WHERE ID_Squadra='$s1' AND ID_Campionato='$c1'";

        }elseif(($ga1+$gr1+$r1)<($ga2+$gr2+$r2)) {
            $sql = "UPDATE QEL SET Eliminato=1 WHERE ID_Squadra='$s1' AND ID_Campionato='$c1'";
            $sql1 = "UPDATE QEL SET Eliminato=0 WHERE ID_Squadra='$s2' AND ID_Campionato='$c2'";
        }
        $result = $conn->query($sql);
        $result1 = $conn->query($sql1);
        $conn->close();
