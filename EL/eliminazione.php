...
$sql = "SELECT count(*) as total FROM partitagirone where coppa=1";
...
$sqltotal = "SELECT count(*) as total FROM GEL WHERE Eliminato=0";
$resulttotal = mysqli_query($conn, $sqltotal);
$rowtotal = $resulttotal->fetch_assoc();
$totalGEL = $rowtotal['total'];

$sqltotal = "SELECT count(*) as total FROM QEL WHERE Eliminato=0";
$resulttotal = mysqli_query($conn, $sqltotal);
$rowtotal = $resulttotal->fetch_assoc();
$totalQEL = $rowtotal['total'];
...
$sql2 = "SELECT * FROM GEL WHERE Eliminato=0";
...
$sql3 = "SELECT * FROM QEL WHERE Eliminato=0";
...
for ($i = 0; $i < count($gironi); $i++) {
    ...
    $sql4 = "SELECT squadra.* FROM statistichegirone, squadra WHERE squadra.ID=statistichegirone.ID_Squadra AND squadra.ID_Campionato=statistichegirone.ID_Campionato AND Coppa = 1 AND Girone = '$gironi[$i]' ORDER BY Pt DESC, DR DESC, GF DESC LIMIT 2 OFFSET 2";
    ...
    while ($row4 = $result4->fetch_assoc()) {
        ...
        $sql5 = "UPDATE GEL SET Eliminato=10 WHERE ID_Squadra='$idSquadra' AND ID_Campionato='$idCampionato'";
        $result5 = mysqli_query($conn, $sql5);

        $sql6 = "UPDATE QEL SET Eliminato=10 WHERE ID_Squadra='$idSquadra' AND ID_Campionato='$idCampionato'";
...
<div class="container">
    <h1 class="mt-5">Eliminazione Diretta Europa League</h1>
            ...
            <tr>
                ...
                <th id="colsim" class="text-center">
                    <form class="mb-0" method="get" action="simulaeliminazione.php">
                        <input type="hidden" name="coppa" value="1">
