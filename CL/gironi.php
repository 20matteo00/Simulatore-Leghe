...
$sql = "SELECT count(*) as total FROM QCL where Eliminato=0";
...
$sql = "SELECT squadra.*, campionato.Nome as nomecampionato FROM GCL, squadra, campionato WHERE squadra.ID = GCL.ID_squadra AND GCL.ID_Campionato = squadra.ID_Campionato AND squadra.ID_Campionato=campionato.ID ORDER BY squadra.Forza DESC, squadra.Nome ASC";
...
$sql = "SELECT squadra.*, campionato.Nome as nomecampionato FROM QCL, squadra, campionato WHERE QCL.Eliminato = 0 AND squadra.ID = QCL.ID_squadra AND QCL.ID_Campionato = squadra.ID_Campionato AND squadra.ID_Campionato=campionato.ID ORDER BY squadra.Forza DESC, squadra.Nome ASC";
$result = $conn->query($sql);
$squadre2 = array();
while ($row = $result->fetch_assoc()) {
    $squadre2[] = $row;
                ...
                <div class="card-body">
                    <table class="table table-striped mb-0">
                        ...
                        <tbody>
                            ...
                            for ($i = 0; $i < 8; $i++) {
                                echo '<tr>';
                                echo '<td>' . $squadre2[$i]['Nome'] . '</td>';
                                echo '<td>' . $squadre2[$i]['nomecampionato'] . '</td>';
                                echo '<td>' . $squadre2[$i]['Forza'] . '</td>';
                                echo '</tr>';
                            }
                            ?>
                        </tbody>
...
if (isset($_POST['genera_gironi'])) {
    ...
    $squadreFascia3 = array_slice($squadre, 16, 8);
    $squadreFascia4 = $squadre2;
    ...
    for ($i = 0; $i < 8; $i += 4) {
        ...
        for ($j = $i; $j < $i + 4; $j++) {
            ...
            foreach ($gironi[$j] as $squadra) {
                ...
                $sqlInsert = "INSERT INTO squadragirone (ID_squadra, ID_Campionato, Coppa, Girone) VALUES ($squadraID, $campionatoID, 0, '$girone')";
