...
if(isset($_GET['simulaeuropa'])){
    ...
    if(isset($_GET['s1']) && isset($_GET['s2']) && isset($_GET['c1']) && isset($_GET['c2'])){
        ...
        $sql = "SELECT squadra.*
                ...
                AND (QECL.Eliminato = 0 OR QECL.Eliminato > 15)
...
function calcola($s1, $s2, $c1, $c2, $coppa) {
    ...
    $sql1 = "SELECT * from squadra WHERE squadra.ID = '$s1' AND squadra.ID_Campionato = '$c1'";
        ...
        if(($ga1+$gr1+$r1)>($ga2+$gr2+$r2)){
            $sql = "UPDATE QECL SET Eliminato=25 WHERE ID_Squadra='$s2' AND ID_Campionato='$c2'";
            $sql2 = "UPDATE QECL SET Eliminato=0 WHERE ID_Squadra='$s1' AND ID_Campionato='$c1'";

        }elseif(($ga1+$gr1+$r1)<($ga2+$gr2+$r2)) {
            $sql = "UPDATE QECL SET Eliminato=25 WHERE ID_Squadra='$s1' AND ID_Campionato='$c1'";
