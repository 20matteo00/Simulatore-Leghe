...
$sql = "SELECT count(*) as total FROM QECL where Eliminato=0";
...
$sql = "SELECT squadra.*, campionato.Nome as nomecampionato FROM QECL, squadra, campionato WHERE squadra.ID = QECL.ID_squadra AND QECL.ID_Campionato = squadra.ID_Campionato AND squadra.ID_Campionato=campionato.ID AND QECL.Eliminato=0 ORDER BY squadra.Forza DESC, squadra.Nome ASC";
...
if (isset($_POST['genera_gironi'])) {
    ...
    $squadreFascia4 = array_slice($squadre, 24, 8);
    ...
    for ($i = 0; $i < 8; $i += 4) {
        ...
        for ($j = $i; $j < $i + 4; $j++) {
            ...
            foreach ($gironi[$j] as $squadra) {
                ...
                $sqlInsert = "INSERT INTO squadragirone (ID_squadra, ID_Campionato, Coppa, Girone) VALUES ($squadraID, $campionatoID, 2, '$girone')";
